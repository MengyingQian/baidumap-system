...
function hotMap(data){
    var calPoints = data.calPoints;
    var basePoints = data.basePoints;
    var number_BS = calPoints.length;
    var props=['站高(m)','DownTilt(度)','Azimuth(度)','载波频点(MHz)','基站发射功率(dBm)'];
    var input = [];
    var result = [];//最终结果
    for(var i=0,length1=calPoints.length;i<length1;i++){
        var temp = [];
        temp.push(calPoints[i][0],calPoints[i][1]);
        for(var j=0,length2=basePoints[i].length;j<length2;j++){
            temp.push(basePoints[i][j].geom.coordinates[0],basePoints[i][j].geom.coordinates[1]);
            for(var k=0;k<props.length;k++){
                temp.push(basePoints[i][j][props[k]]);
            }
        }
        input.push(temp); 
    }
    ...
    for (var i=0;i<number_BS;i++){
        ...
        result[i] = {
            lng: calPoints[i][0],
            lat: calPoints[i][1],
            count: 10*Math.log(power_re[i])/Math.log(10)-20*Math.log(f)/Math.log(10)-75.06
        }; 
    }
    return result;
}
...
function layout(data){
    ...
    for (i=0;i<result[0].length;i++){
        ...
        rData[i]['rePoints'].splice(rData[i]['rePoints'].indexOf(0));
    }
    return rData;
}
module.exports={
